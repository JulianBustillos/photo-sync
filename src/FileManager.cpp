...
FileManager::FileManager(QObject *parent)
    : QThread(parent), m_extensions({"*.jpg", "*.jpeg", "*.png", "*.mp4"}),
      m_cancelled(false), m_status(false), m_removeFiles(false), m_runCount(0),
      m_progress(0), m_copyProgress(0), m_removeProgress(0),
      m_duplicateCount(0), m_copyCount(0), m_removeCount(0) {}
...
void FileManager::setSettings(const QString &importPath,
                              const QString &exportPath, bool removeFiles) {
...
bool FileManager::checkDir() {
  ...
  if (!m_importDir.exists()) {
    emit warning("Path error",
                 "Import path : \"" + m_importDir.absolutePath() +
                     "\" not found !",
                 false);
    return false;
  }
  ...
  if (!m_exportDir.exists()) {
    emit warning("Path error",
                 "Export path : \"" + m_exportDir.absolutePath() +
                     "\" not found !",
                 false);
    return false;
  }
...
bool FileManager::checkRemove() {
  if (m_removeFiles) {
    QMutexLocker locker(&m_mutex);
    emit warning("Remove warning",
                 "Files are going to be removed after copy. Proceed anyway ?",
                 true);
...
void FileManager::exportFiles() {
  ...
  for (ExportFile &fileToCopy : m_filesToCopy) {
    ...
    QFileInfo exportFileInfo(m_exportDir.absolutePath() +
                             fileToCopy.m_date.toQString() + "/" + fileName);
    ...
    if (exportFileInfo.exists()) {
      ...
      while (exportFileInfo.exists() && count < MAX_NAME_INDEX) {
        exportFileInfo.setFile(name + "_" + QString::number(++count) +
                               extension);
      }
    }
...
void FileManager::printStats() {
  if (m_duplicateCount > 0)
    emit output("Found " + QString::number(m_duplicateCount) +
                (m_duplicateCount > 1 ? " already existing files."
  ...
  if (m_removeCount > 0) {
    emit output(QString::number(m_removeCount) +
                (m_removeCount > 1 ? " files removed." : " file removed."));
  }
