...
PhotoSync::PhotoSync(QWidget *parent)
    : QMainWindow(parent), m_settings(QCoreApplication::applicationDirPath()),
      m_fileManager(this) {
  m_ui.setupUi(this);
  ...
  QObject::connect(&m_fileManager, &FileManager::output, this,
                   &PhotoSync::appendOutput);
  QObject::connect(&m_fileManager, &FileManager::finished, this,
                   &PhotoSync::finish);
  QObject::connect(this, &PhotoSync::warningAnswer, &m_fileManager,
                   &FileManager::warningAnswer);
}
...
void PhotoSync::selectDirectory(QString title, QLineEdit &lineEdit) {
  QString startDir =
      !lineEdit.text().isEmpty() ? lineEdit.text() : QDir::homePath();
  QString selectedDir =
      QFileDialog::getExistingDirectory(this, title, startDir);
...
void PhotoSync::createWarning(QString title, QString message, bool emitAnswer) {
  QMessageBox::StandardButton button = QMessageBox::warning(
      this, title, message,
      emitAnswer ? QMessageBox::Ok | QMessageBox::Cancel : QMessageBox::Ok);
  if (emitAnswer)
